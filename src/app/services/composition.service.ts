...
export class CompositionService {
  ...
  private async renderLayersToContext(ctx: CanvasRenderingContext2D, layers: CompositionLayer[]): Promise<void> {
    ...
    for (const layer of sortedLayers) {
      ...
      if (layer.customDither?.enabled && !layer.ditherExempt) {
        imageData = await this.applyCustomDither(imageData, layer.customDither);
      }
   ...
   */
  private async applyCustomDither(
    imageData: ImageData,
    customDither: {
      algorithm: string;
      palette?: string;
    ...
    return this.ditheringService.applyDitheringAsync(imageData, customOptions);
  }
